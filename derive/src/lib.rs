        ...
        impl async_graphql_relay::RelayNodeInterface for Node {
            async fn fetch_node(ctx: async_graphql_relay::RelayContext, relay_id: String) -> Result<Self, async_graphql::Error> {
                ...
                if let None = split_position {
                    return Err(async_graphql::Error::new("Invalid id provided to node query! RelayNodId should contain '+' and a seperator!"));
                }
                let splited = relay_id.split_at(split_position.unwrap());
                let id = splited.0;
                if id.len() != 36 {
                    return Err(async_graphql::Error::new("Invalid id provided to node query! Uuid v4 length should be 36!"));
                }
